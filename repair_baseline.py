...
def parseArgs():
    parser = ArgumentParser(description="Multithreaded Repair Baseline Mode")
    parser.add_argument("--log", action="store_true", help="Enable terminal logging")
    
    # 改為接收目錄路徑，並提供預設值
    parser.add_argument("--dirpath", required=True, help="Directory containing the JSON files to repair")
...
def runRepairTask(file_path, args):
    ...
    try:
        # 建立 Repair 策略
        strategy = RepairOnlyOneLanguage(strategy_config, log, target_file)

        # 放入執行環境並啟動
        context = RunContext()
        context.setStrategy(strategy)
        
        # 這裡會自動掃描錯誤、重打 API，並把結果覆寫回 JSON 檔
...
def main():
    ...
    print(f"🚀 準備執行批次修復作業 (Run Batch Repair)...")
    ...
    print("\n✅ 所有修復作業完成！你可以安心執行算分成式碼 (TestEM) 了。")
