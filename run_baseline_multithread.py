...
def parseArgs():
    ...
    # 1. 將 nargs 設定為 '+', 讓這些參數可以接收多個值，變成一個 List
    ...
    # 2. 新增 workers 參數，讓你可以控制最大執行緒數量，避免踩到 API Rate Limit
    parser.add_argument("-w", "--workers", type=int, default=3, help="Max concurrent threads/workers")

    args = parser.parse_args()
    return args

def runExperiment(model_name, dataset_name, language, args, print_lock):
    """
    ...
    # 3. 使用 print_lock 確保輸出不會跟其他執行緒的字元混在一起
    with print_lock:
...
def main():
    ...
    # 4. 產生所有參數的排列組合 (Cartesian Product)
    # 例如：2 個模型 x 3 個資料集 x 2 個語言 = 12 個任務
    ...
    # 建立一個 Lock，專門用來排隊列印，避免終端機畫面大亂
    print_lock = threading.Lock()

    # 5. 啟動 ThreadPoolExecutor
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = []
        for model_name, dataset_name, language in tasks:
            # 將任務提交給執行緒池
            futures.append(
                executor.submit(runExperiment, model_name, dataset_name, language, args, print_lock)
            )
