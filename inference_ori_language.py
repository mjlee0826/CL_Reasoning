...
def main():
    ...
    for x_dict, y_list, meta in tqdm(zip(val_X, val_y, val_meta), total=len(val_X), desc="Predicting Validation Set"):
        ...
        for lang, question_text in x_dict.items():
            ...
            # 🎯 核心修改：過濾出「包含當前輸入語言 (lang)」的語言配對
            # pair 的格式為 "english_vs_chinese"，我們用 split 切開來比對
            valid_probs_dict = {
                pair: prob 
                for pair, prob in probs_dict.items() 
                if lang in pair.split("_vs_")
            }
            
            # 從「有參與的 language pair」中，選出機率最高的那一個
            # 增加防呆：萬一找不到對應語言 (理論上不可能發生)，就 fallback 到全域最高
            if valid_probs_dict:
                best_pair = max(valid_probs_dict, key=valid_probs_dict.get)
            else:
            ...
            final_results.append({
                ...
                "predicted_probabilities": probs_dict,  # 完整機率依舊保留，方便事後觀察
